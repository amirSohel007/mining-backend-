...
app.post('/', async (req, res) => {
    ...
    try {
        let { title = null, description, price = 0, daily_income = 0, active = true, plan_id = null } = req.body;
        ...
        if (user_id) {
            if (plan_id) {
                const result = await updateSubscriptionPlan(user_id, plan_id, { title, description, price, active, daily_income });
